...
get_metrics <- function(predicted, actual, positive_class = "1") {
  cm <- confusionMatrix(as.factor(predicted), as.factor(actual), positive = positive_class)
  return(data.frame(
...
# Logistic Regression
metrics_logistic <- get_metrics(pred_logistic_class, test_data$CLASS_LABEL)

# Make sure KNN predicted and actual labels are factors with correct levels
knn_model_class <- factor(knn_model_class, levels = c("Class1", "Class2"))
test_label_knn_class <- factor(test_label_knn_class, levels = c("Class1", "Class2"))

# Now get the metrics with Class1 as the positive class
metrics_knn <- get_metrics(knn_model_class, test_label_knn_class, positive_class = "Class1")

# Naive Bayes
metrics_nb <- get_metrics(pred_nb, test_label_nb)

# Decision Tree
metrics_dt <- get_metrics(pred_dt, test_label_dt)

# Random Forest
metrics_rf <- get_metrics(pred_rf, test_label_rf)
